  ...
  return (
    <section className="flex w-full items-center justify-center pb-20">
      <div className=" max-w-7xl">
        <h1
          className={`my-6 text-center sm:my-9  ${isMobile ? "text-headlineSmall" : "text-displaySmall"}`}
        >
        ...
        <div
          className={`flex  justify-center gap-7 ${isMobile ? "flex-col items-center" : "sm:flex-row sm:items-stretch"}`}
        >
          {events.map((event) => (
            <div
              key={event.title}
              className={`flex w-72 flex-col gap-5 bg-white pb-5 sm:w-96 ${isMobile ? "rounded-[32px]" : "rounded-[50px]"}`}
            >
              <Image
              ...
              <h3
                className={`text-center ${isMobile ? "text-headlineSmall" : "text-headlineLarge"}`}
              >
                {translations["tasting-info." + event.title]}
              </h3>
              <div
                className={`flex flex-col justify-between px-3 ${isMobile ? "h-32" : "h-44"}`}
              >
                <p
                  className={`text-center ${isMobile && "text-labelSmall"}`}
                  dangerouslySetInnerHTML={{
                    __html: translations["tasting-info-card." + event.body],
                  }}
                ></p>
                <p className={`text-center ${isMobile && "text-labelSmall"}`}>
                  {translations["tasting-info-card." + event.engagement]}
                </p>
              </div>
