...
const WineCard = ({ title, price, imageUrl, isNew, variant }) => {
  ...
  return (
    <section
      ...
      className={`group box-border flex h-full w-full cursor-pointer flex-col items-center justify-center gap-2 rounded-lg bg-tertiary2-light p-4 shadow-lg transition-transform duration-300 hover:scale-105 ${responsiveStyles.section}`}
    >
      ...
      <div
        className={`relative flex h-[283px] flex-col items-center justify-center ${responsiveStyles.image}`}
      >
        <img
          src={imageUrl}
